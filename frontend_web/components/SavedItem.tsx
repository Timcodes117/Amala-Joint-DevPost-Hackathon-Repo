...
function SavedItem({
  ...
  return (
    <div className="w-full min-h-fit !border-b-gray-600/90 p-2 overflow-hidden  relative border-b flex flex-row gap-2">
      {/* Image + favorite */}
      <div className="relative w-[100px] h-[100px] min-w-[100px] bg_3 rounded-[12px] overflow-hidden">
        ...
        {/* Verified badge (kept hidden like source) */}
        {/* {verified ? (
      ...
      <div className=" w-full">
        <div className="flex items-start justify-between gap-2 w-full">
          <b className="text-[18px] md:text-[20px] font-semibold line-clamp-1 flex items-center flex-wrap">
            {/* <span className="inline-flex items-center gap-1 line-clamp-1"> */}
              {name}
              {/* <HiCheckBadge className='pry-color ml-1 min-w-[18px]' size={18}  /> */}
            {/* </span> */}
          </b>
          {/* Favorite button (active for saved list) */}
          <button
            onClick={onToggleFav}
            aria-label="favorite"
            className="  top-4 right-4 h-10 min-w-10 rounded-full bg-black/50 backdrop-blur flex items-center justify-center"
          >
            <Heart
              size={18}
